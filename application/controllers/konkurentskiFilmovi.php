...
class KonkurentskiFilmovi extends PreController 
{
	...
	public function read()
	{
		...
		$totalRows = $this->db->get( 'konkurentski_filmovi_novo' )->num_rows();
		
		$this->db->select( 'konkurentski_filmovi_novo.*' );	
		$this->db->order_by(  'id', 'desc');
		$query = $this->db->get('konkurentski_filmovi_novo', $this->_limit, $this->getRowsOffset( $this->_page, $this->_limit ) );
